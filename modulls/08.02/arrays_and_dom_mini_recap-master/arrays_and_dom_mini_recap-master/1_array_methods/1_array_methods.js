...
const friendsJohn = data.filter((person) => person.name === "John Doe");
console.log(friendsJohn[0].friends);
// Exercise 2: Use the filter method to get all the people who live in New York
const peopleInNY = data.filter((person) => person.address.city === "New York");
console.log(peopleInNY);
...
const johnsFriends = data.flatMap((item) => {
  let arr = [];
  if (item.name === `John Doe`) {
    arr = item.friends.flatMap((item) => {
      return item;
    });
  }
  return arr;
...
const allAddresses = data.flatMap((item) => {
  let newArray = [];
});
...
const chicago = data.find((element) => element.address.city === "Chicago");
console.log(chicago);
// Exercise 2: Use the find method to find the first person who is older than 30
const older30 = data.find((element) => element.age > 30);
console.log(older30);
// Exercise 3: Use the find method to find the first person who has "reading" as a hobby
const reading = data.find((element) => element.hobbies.includes("reading"));
console.log(reading);
// FOREACH
...
data.forEach((element) => {
  ...
  let friendName = element.friends;
  friendName.forEach((element) => {
    console.log(element.name);
  });
});
// Exercise 2: Use the forEach method to add a new property "isAdult" to each person object and
// set it to true if the person is over 18 and false if not
// data.forEach((element) => {
//   let isAdult = { asd: 12 };
//   Object.assign(isAdult, element);
// });
// console.log(element);
...
const cooking = data.some((person) => person.hobbies.includes("cooking"));
console.log(cooking);
// Exercise 2: Use the some method to check if any of the people in the data array live in California
const cal = data.some((person) => person.address.city === "Califonia");
console.log(cal);
...
const countState = data.reduce((acu, cur) => {
  if (!acu.hasOwnProperty(cur.address)) {
    acu[cur.address.state] = 1;
  } else {
    acu[cur.address.state] += 1;
  }
  return acu;
...
let avg = data.reduce((acc, person) => acc + person.age / data.length, 0);
console.log(avg);
...
const asd = data.map((person) => person.hobbies).includes("gaming");
console.log(asd);
